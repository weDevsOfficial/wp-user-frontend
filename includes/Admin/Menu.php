...
class Menu {
    ...
    public function admin_menu() {
        ...
        $post_forms_hook  = add_submenu_page('wp-user-frontend', __('Post Forms', 'wp-user-frontend'), __('Post Forms', 'wp-user-frontend'), $capability, 'wpuf-post-forms', [ $this, 'wpuf_post_forms_page' ]);
        ...
        foreach ( $this->all_submenu_hooks as $submenu_hook ) {
            add_action( $submenu_hook, [ $this, 'enqueue_common_admin_scripts' ] );
        }

        add_action( 'load-' . $this->all_submenu_hooks['post_forms'], [ wpuf()->admin_form, 'post_forms_builder_init' ] );
    }

    public function enqueue_common_admin_scripts() {
    ...
    public function set_main_menu_hook( $hook ) {
        $this->main_menu_hook = $hook;
    }

    public function get_main_menu_hook() {
        return $this->main_menu_hook;
    }
