...
export default function SubscriptionBox({ subscription }) {
    ...
    const { getReadableBillingAmount, isRecurring: getIsRecurring } = useSelect((select) => {
        const store = select('wpuf/subscriptions');
        return {
            getReadableBillingAmount: (sub) => store.getReadableBillingAmount(sub),
            isRecurring: (sub) => store.isRecurring(sub),
        };
    }, []);
    ...
    const isRecurring = useMemo(() => getIsRecurring(subscription), [subscription, getIsRecurring]);
    ...
    const subscribersLink = useMemo(() => {
        const wpufSubscriptions = window.wpufSubscriptions || {};
        return `${wpufSubscriptions.siteUrl}/wp-admin/edit.php?post_type=wpuf_subscription&page=wpuf_subscribers&post_ID=${subscription.ID}`;
    }, [subscription.ID]);
    ...
    const processPromiseResult = async (promiseResult) => {
        try {
            ...
            if (result.success) {
                ...
                setItem(null);
                const currentStatus = subscription.post_status === 'trash' ? 'trash' : 'all';
                await setSubscriptionsByStatus(currentStatus);
                getSubscriptionCount();
            } else {
                addNotice({
                    type: 'danger',
            ...
            addNotice({
                type: 'danger',
                message: __('An error occurred', 'wp-user-frontend'),
            });
        }
    };

    const trashSubscription = async () => {
        ...
        const promiseResult = updateItem();
        await processPromiseResult(promiseResult);
    };
    ...
    const deleteSubscription = async () => {
        const promiseResult = deleteItem(subscription.ID);
        await processPromiseResult(promiseResult);
    };
    ...
    return (
        <>
            <div className="wpuf-text-base wpuf-justify-between wpuf-bg-white wpuf-border wpuf-border-gray-200 wpuf-rounded-xl wpuf-shadow wpuf-relative">
                <div
                    onClick={() => subscription.post_status !== 'trash' && handleEdit()}
                    className={`wpuf-flex wpuf-justify-between wpuf-border-b border-gray-900/5 wpuf-bg-gray-50 wpuf-p-6 wpuf-rounded-t-xl ${
                        subscription.post_status !== 'trash' ? 'wpuf-cursor-pointer' : ''
                    }`}
                >
                    <div>
                        <div
                            className="wpuf-flex wpuf-py-1 wpuf-text-gray-900 wpuf-m-0 wpuf-font-medium"
                        ...
                        <p className="wpuf-text-gray-500 wpuf-text-base wpuf-m-0">
                            {getReadableBillingAmount(subscription)}
                        </p>
                    </div>
                </div>
                <div className="wpuf-absolute wpuf-right-6 wpuf-p-[10px] wpuf-rounded-full hover:wpuf-bg-gray-100 wpuf-top-4 wpuf-right-4" ref={menuRef}>
                    <svg
                        onClick={() => setQuickMenuStatus(!quickMenuStatus)}
                        ...
                        <div className="wpuf-w-max wpuf--left-20 wpuf-absolute wpuf-rounded-xl wpuf-bg-white wpuf-shadow-lg wpuf-ring-1 wpuf-ring-gray-900/5 wpuf-overflow-hidden wpuf-z-10">
                            {subscription.post_status !== 'trash' ? (
                                <ul>
                                    <li
                                        onClick={handleEdit}
                                        className="wpuf-px-4 wpuf-py-2 wpuf-mb-0 hover:wpuf-bg-gray-100 hover:wpuf-cursor-pointer"
                                    >
                                    ...
                                    <li
                                        ...
                                        className="wpuf-px-4 wpuf-py-2 wpuf-mb-0 hover:wpuf-bg-gray-100 hover:wpuf-cursor-pointer"
                                    >
                                        {subscription.post_status === 'publish'
                ...
                <div className="wpuf-flex wpuf-px-6 wpuf-py-6 wpuf-justify-between wpuf-items-center">
                    <div className={`${pillColor} wpuf-text-sm wpuf-w-fit wpuf-px-2.5 wpuf-py-1 wpuf-shadow-sm wpuf-rounded-md wpuf-border`}>
                        {postStatus}
                    </div>
                    {isRecurring && (
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="wpuf-flex wpuf-px-6 wpuf-pb-6 wpuf-justify-between wpuf-items-center">
                    <p className="wpuf-text-gray-500 wpuf-text-sm wpuf-m-0">{__('Total Subscribers')}</p>
                    <a href={subscribersLink} className="wpuf-text-gray-500">
                        {subscribers}
                    </a>
                </div>
            </div>
            {showPopup && (
                <Popup
                    ...
                    onTrashSubscription={trashSubscription}
