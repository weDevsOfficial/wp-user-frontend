...
<div class="wrap wpuf-ai-form-wrapper" id="wpuf-ai-form-generating">
    <div class="wpuf-ai-form-container">
        <div class="wpuf-ai-form-content wpuf-bg-white wpuf-rounded-lg">
            <!-- Animated Icon -->
            <div class="wpuf-flex wpuf-justify-center">
            ...
            <!-- Title -->
            <h3 class="wpuf-text-center wpuf-text-xl wpuf-font-base wpuf-text-gray-900">
                <?php esc_html_e( 'Generating your form...', 'wp-user-frontend' ); ?>
            </h3>
            ...
            <div class="wpuf-grid wpuf-grid-cols-1 wpuf-justify-items-center wpuf-gap-1">
                ...
                <div class="wpuf-flex wpuf-items-center wpuf-gap-3 wpuf-justify-center wpuf-w-full wpuf-max-w-md" :class="{ 'wpuf-opacity-100': currentStep >= 1, 'wpuf-opacity-40': currentStep < 1 }">
                    ...
                    <p class="wpuf-text-gray-600 wpuf-text-sm wpuf-leading-8 wpuf-flex-1"><?php esc_html_e( 'Analyzing your request and detecting the form type...', 'wp-user-frontend' ); ?></p>
                </div>
                ...
                <div class="wpuf-flex wpuf-items-center wpuf-gap-3 wpuf-justify-center wpuf-w-full wpuf-max-w-md" :class="{ 'wpuf-opacity-100': currentStep >= 2, 'wpuf-opacity-40': currentStep < 2 }">
                    ...
                    <p class="wpuf-text-gray-600 wpuf-text-sm wpuf-leading-8 wpuf-flex-1"><?php esc_html_e( 'Finalizing the title, required fields, and labels...', 'wp-user-frontend' ); ?></p>
                </div>
                ...
                <div class="wpuf-flex wpuf-items-center wpuf-gap-3 wpuf-justify-center wpuf-w-full wpuf-max-w-md" :class="{ 'wpuf-opacity-100': currentStep >= 3, 'wpuf-opacity-40': currentStep < 3 }">
                    ...
                    <p class="wpuf-text-gray-600 wpuf-text-sm wpuf-leading-8 wpuf-flex-1"><?php esc_html_e( 'Almost done! Generating your form preview...', 'wp-user-frontend' ); ?></p>
                </div>
                ...
                <div class="wpuf-flex wpuf-items-center wpuf-gap-3 wpuf-justify-center wpuf-w-full wpuf-max-w-md" :class="{ 'wpuf-opacity-100': currentStep >= 4, 'wpuf-opacity-40': currentStep < 4 }">
                    ...
                    <p class="wpuf-text-gray-600 wpuf-text-sm wpuf-leading-8 wpuf-flex-1"><?php esc_html_e( 'Here\'s your AI-generated form - ready to customize and use!', 'wp-user-frontend' ); ?></p>
                </div>
            </div>
            
            <!-- Confetti Animation -->
            <div v-if="showConfetti" class="wpuf-confetti-container">
                <img src="<?php echo esc_url( plugin_dir_url( dirname( dirname( dirname( __FILE__ ) ) ) ) . 'assets/images/confetti_transparent.gif' ); ?>" alt="Confetti" class="wpuf-confetti-gif">
            </div>
        </div>
    </div>
</div>
...
.wpuf-ai-form-content p {
    margin: 0.25em 0 0.25em 0 !important;
}

.wpuf-ai-form-content h3 {
    margin: 0 !important;
}

.wpuf-confetti-container {
    position: absolute;
    top: 50%;
    left: 50%;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Vue({
        el: '#wpuf-ai-form-generating',
        
        data: {
            currentStep: 1,
            showConfetti: false,
            description: '<?php echo esc_js( isset( $_GET['description'] ) ? sanitize_text_field( wp_unslash( $_GET['description'] ) ) : '' ); ?>',
            prompt: '<?php echo esc_js( isset( $_GET['prompt'] ) ? sanitize_text_field( wp_unslash( $_GET['prompt'] ) ) : '' ); ?>'
        },
        ...
        methods: {
            startGeneration: function() {
                ...
                // After completion, show confetti and redirect to form builder
                setTimeout(function() {
                    self.currentStep = 5; // Mark as completed
                    self.showConfetti = true; // Show confetti animation
                }, 6000);
                
                setTimeout(function() {
                    // Redirect to success page
                    var successUrl = 'admin.php?action=wpuf_ai_form_success';
                    
                    // Add form parameters if available
                    if (self.description) {
                        successUrl += '&form_title=' + encodeURIComponent(self.description);
                    }
                    
                    // This would include the actual form ID from the API response
                    // successUrl += '&form_id=' + formId;
