...
export default function InfoCard() {
    const { item, getReadableBillingAmount, isRecurring: getIsRecurring } = useSelect((select) => {
        const store = select('wpuf/subscriptions');
        return {
            item: store.getItem(),
            getReadableBillingAmount: (sub, returnAsHtml) => store.getReadableBillingAmount(sub, returnAsHtml),
    ...
    return (
        <div className="wpuf-mt-4 wpuf-border wpuf-border-gray-200">
            <dl className="wpuf-mx-auto wpuf-grid bg-gray-900/5 wpuf-grid-cols-4 wpuf-border-b-2 wpuf-border-dashed wpuf-bg-white wpuf-p-2">
                <div
                    className="wpuf-flex wpuf-col-span-2 wpuf-flex-wrap wpuf-items-baseline wpuf-justify-between wpuf-py-2 wpuf-px-6"
                    title={`id: ${item.ID}`}
                >
                    <dt className="wpuf-text-sm wpuf-font-medium wpuf-leading-6 wpuf-text-gray-500">
                        {__('Plan', 'wp-user-frontend')}
                    </dt>
                    <dd className="wpuf-w-full wpuf-flex-none wpuf-text-2xl wpuf-leading-10 wpuf-tracking-tight wpuf-text-gray-900">
                        {item.post_title} --- 
                    </dd>
                </div>
                <div className="wpuf-flex wpuf-flex-wrap wpuf-items-baseline wpuf-justify-between wpuf-px-4 wpuf-py-2">
                    <dt className="wpuf-text-sm wpuf-font-medium wpuf-leading-6 wpuf-text-gray-500">
                        {__('Payment', 'wp-user-frontend')}
                    </dt>
                    <dd
                        className="wpuf-w-full wpuf-flex-none wpuf-text-2xl wpuf-leading-10 wpuf-tracking-tight wpuf-text-gray-900"
                        dangerouslySetInnerHTML={{ __html: getReadableBillingAmount(item, true) }}
                    ></dd>
                </div>
                {isRecurring && (
                    <div className="wpuf-flex wpuf-flex-wrap wpuf-items-baseline wpuf-justify-between wpuf-px-4 wpuf-py-5">
                        <dt className="wpuf-text-sm wpuf-italic wpuf-font-medium wpuf-leading-6 wpuf-text-gray-500 wpuf-flex wpuf-items-center wpuf-justify-center">
            ...
            <dl className="wpuf-mx-auto wpuf-grid wpuf-grid-cols-1 bg-gray-900/5 wpuf-bg-white wpuf-p-2">
                <div className="wpuf-flex wpuf-flex-wrap wpuf-items-baseline wpuf-justify-between wpuf-bg-white wpuf-px-4 wpuf-py-2">
                    <dt className="wpuf-text-sm wpuf-font-medium wpuf-leading-6 wpuf-text-gray-500">
                        {__('Subscribers', 'wp-user-frontend')}
                    </dt>
                    <dd className="wpuf-flex wpuf-items-center wpuf-w-full wpuf-flex-none wpuf-text-2xl wpuf-leading-10 wpuf-tracking-tight wpuf-text-gray-900">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
