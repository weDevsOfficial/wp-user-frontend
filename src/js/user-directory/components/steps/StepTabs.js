...
const ProBadge = ({ width = '39px', height = '22px', utm = 'wpuf-user-directory-tabs' }) => {
...
const StepTabs = ({ formData, setFormData, config }) => {
    ...
    const handleKeyDown = (e, index) => {
        ...
        // Arrow up/down with modifier to reorder
        if (isModified && (key === 'ArrowUp' || key === 'ArrowDown')) {
            e.preventDefault();
            const newIndex = key === 'ArrowUp' ? index - 1 : index + 1;

            if (newIndex >= 0 && newIndex < allTabs.length) {
                const newTabs = [...allTabs];
                const movedTab = newTabs[index];
        ...
        // Arrow keys without modifier for navigation
        else if (key === 'ArrowUp' || key === 'ArrowDown') {
            e.preventDefault();
            const newIndex = key === 'ArrowUp' ? Math.max(0, index - 1) : Math.min(allTabs.length - 1, index + 1);
            setFocusedIndex(newIndex);

            // Focus the new item
            const nextElement = e.currentTarget.parentElement?.children[newIndex];
            if (nextElement) {
                nextElement.focus();
            }
        }
        // Home/End keys for navigation
            ...
            e.currentTarget.parentElement?.children[lastIndex]?.focus();
        }
    };
    ...
    return (
        <div className="wpuf-w-full wpuf-bg-white wpuf-max-w-3xl wpuf-mx-auto">
            ...
            <div className="wpuf-mt-8">
                <h3 className="wpuf-text-lg wpuf-font-normal wpuf-text-gray-900 wpuf-m-0 wpuf-mb-2">
                ...
                {/* Draggable Tabs */}
                <div
                    className="wpuf-space-y-2"
                    role="list"
                    aria-label={__('Profile tabs list. Use Ctrl/Cmd + Arrow Up/Down to reorder tabs', 'wp-user-frontend')}
                >
                    {allTabs.map((tab, index) => {
                        ...
                        return (
                            <div
                                ...
                                role="listitem"
                                tabIndex={0}
                                aria-label={`${tab.label} tab. Position ${index + 1} of ${allTabs.length}. Press Ctrl/Cmd + Arrow Up or Down to reorder.`}
                                className={`
                                    ...
                                    ${focusedIndex === index ? 'wpuf-ring-2 wpuf-ring-primary wpuf-ring-offset-2' : ''}
                                `}
                                draggable={true}
                            ...
                            >
                                ...
                                <div className="wpuf-flex wpuf-items-center wpuf-gap-3 wpuf-relative">
                                    ...
                                    <svg
                                        ...
                                        aria-disabled="true"
                                        aria-label={__('Edit tab (Pro feature)', 'wp-user-frontend')}
                                        role="button"
                                    >
                                    ...
                                    <div
                                        ...
                                        aria-checked="true"
                                        aria-disabled="true"
                                        aria-label={__('Toggle tab visibility (Pro feature, always enabled in free version)', 'wp-user-frontend')}
                                        style={{
                                            width: '36px',
                                            height: '16px',
                                            borderRadius: '8px',
            ...
            <div className="wpuf-mt-8">
                ...
                {/* Add Content Button - Disabled with Pro Badge inside */}
                <button
                    type="button"
                    className="wpuf-text-white wpuf-font-medium wpuf-transition-colors wpuf-flex wpuf-items-center wpuf-cursor-not-allowed wpuf-opacity-60 hover:wpuf-opacity-80"
                    style={{
                        height: '42px',
                        borderRadius: '6px',
                        paddingTop: '9px',
                ...
                >
                    ...
                    <ProBadge utm="wpuf-user-directory-add-content" />
                </button>
                ...
                <div className="wpuf-mt-4 wpuf-p-4 wpuf-bg-white wpuf-border wpuf-border-gray-200 wpuf-rounded-lg" style={{ width: '590px' }}>
                    <div className="wpuf-flex wpuf-gap-2 wpuf-items-center wpuf-justify-center">
                        ...
                        <button
                            type="button"
                            className="wpuf-relative wpuf-flex wpuf-flex-col wpuf-items-center wpuf-justify-center wpuf-rounded-lg wpuf-text-sm wpuf-font-medium wpuf-w-[102px] wpuf-gap-1 wpuf-p-3 wpuf-pb-1 wpuf-bg-gray-50 wpuf-text-gray-400 wpuf-cursor-not-allowed wpuf-opacity-60 hover:wpuf-opacity-80"
                            style={{
