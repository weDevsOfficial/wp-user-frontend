<?php
/**
 * Provides logging capabilities for debugging purposes.
 *
 * @since WPUF_SINCE
 */

defined( 'ABSPATH' ) || exit;

/**
 * WPUF_File_Logger class.
 *
 * This logger writes to a PHP stream resource.
 */
class WPUF_File_Logger {

    /**
     * The single instance of the class.
     *
     * @var WPUF_File_Logger
     * @since WPUF_SINCE
     */
    protected static $_instance = null;

    /**
     * Default log format
     */
    const DEFAULT_LOG_FORMAT = "[%datetime%] %level%: %message%\n";

    /**
     * Default date format
     *
     * Example: 16/Nov/2014:03:26:16 -0500
     */
    const DEFAULT_DATE_FORMAT = 'd/M/Y:H:i:s O';

    /**
     * System is unusable
     */
    const EMERGENCY = 'emergency';

    /**
     * Action must be taken immediately
     *
     * Example: Entire website down, database unavailable, etc. This should
     * trigger SMS/E-mail alerts and wake you up.
     */
    const ALERT = 'alert';

    /**
     * Critical conditions
     *
     * Example: Application component unavailable, unexpected exception.
     */
    const CRITICAL = 'critical';

    /**
     * Runtime errors that do not require immediate action but should typically
     * be logged and monitored.
     */
    const ERROR = 'error';

    /**
     * Exceptional occurrences that are not errors.
     *
     * Example: Use of deprecated APIs, poor use of an API, undesirable things
     * that are not necessarily wrong.
     */
    const WARNING = 'warning';

    /**
     * Normal but significant events.
     */
    const NOTICE = 'notice';

    /**
     * Interesting events.
     *
     * Example: User logs in, SQL logs.
     */
    const INFO = 'info';

    /**
     * Detailed debug information.
     */
    const DEBUG = 'debug';

    /**
     * @var string $file_name The log file name
     */
    private $file_name;

    /**
     * @var resource|null $file_handle The file handle for writing
     */
    private $file_handle;

    /**
     * @var string $level The log level
     */
    private $level = self::INFO;

    /**
     * @var string $log_directory Where logs are stored
     */
    private $log_directory;

    /**
     * @var integer $mode The mode to use if the log file needs to be created
     */
    private $mode = 0640;

    /**
     * Cache logs that could not be written yet.
     *
     * If a log is written too early in the request, pluggable functions may be unavailable. These
     * logs will be cached and written on 'init' action.
     *
     * @var array
     */
    protected $cached_logs = [];

    /**
     * The logger constructor. Initialize the default file and log_directory
     *
     * @since WPUF_SINCE
     *
     * @return void
     */
    protected function __construct() {
        $upload_dir          = wp_get_upload_dir();
        $this->log_directory = apply_filters( 'wpuf_log_directory', $upload_dir['basedir'] . '/wpuf-logs' );

        add_action( 'init', [ $this, 'write_cached_logs' ] );
    }

    /**
     * Main WPUF_File_Logger Instance.
     * Ensures only one instance of this class is loaded or can be loaded.
     *
     * @since WPUF_SINCE
     *
     * @return WPUF_File_Logger Main instance.
     */
    public static function instance() {
        if ( is_null( self::$_instance ) ) {
            self::$_instance = new self();
        }
        return self::$_instance;
    }

    /**
     * Get a log file name.
     * Log file name, followed by the date, followed by a hash, .log.
     *
     * @since WPUF_SINCE
     *
     * @return bool|string The log file name or false if cannot be determined.
     */
    public function get_log_file_name() {
        if ( function_exists( 'wp_hash' ) ) {
            $date_suffix    = gmdate( 'Y-m-d' );
            $hash_suffix    = wp_hash( $this->level );
            $sanitized_name = sanitize_file_name( implode( '-', [ $this->level, $date_suffix, $hash_suffix ] ) . '.log' );
            $log_file_name  = sanitize_file_name( apply_filters( 'wpuf_log_file_name', $sanitized_name ) );

            return $log_file_name;
        } else {
            _doing_it_wrong( __METHOD__, __( 'This method should not be called before plugins_loaded.', 'wp-user-frontend' ), '3.5.29' );
            return false;
        }
    }

    /**
     * Process the message and log it to the logfile.
     *
     * @since WPUF_SINCE
     *
     * @param string          $message  The message to log.
     * @param string          $level    The log level. Default INFO.
     * @param string|resource $log_file Optional. Log file path or resource.
     *
     * @return bool
     */
    public function log( $message, $level = self::INFO, $log_file = null ) {
        // Close any previously opened handle to prevent resource leaks
        $this->close();

        $this->level = $level;

        if ( null !== $log_file ) {
            $this->file_name = $log_file;
        } else {
            $this->file_name = $this->get_log_file_name();
        }

        if ( false === $this->file_name ) {
            $this->cache_log( $message, $level );
            return true;
        }

        $this->open();

        $message = $this->format_message(
            [
                'message'  => $message,
                'level'    => strtoupper( $level ),
                'datetime' => new DateTime(),
            ]
        );

        return $this->write( $message, $level );
    }

    /**
     * Transform log variables into the defined log format.
     *
     * @since WPUF_SINCE
     *
     * @param array $variables The log variables.
     *
     * @return string
     */
    protected function format_message( $variables ) {
        $template = self::DEFAULT_LOG_FORMAT;

        foreach ( $variables as $key => $value ) {
            if ( strpos( $template, '%' . $key . '%' ) !== false ) {
                $template = str_replace(
                    '%' . $key . '%',
                    $this->export( $value ),
                    $template
                );
            }
        }

        return apply_filters( 'wpuf_formatted_log_message', $template );
    }

    /**
     * Exports a PHP value for logging to a string.
     *
     * @since WPUF_SINCE
     *
     * @param mixed $value The value to export.
     *
     * @return string
     */
    protected function export( $value ) {
        if ( $value instanceof DateTime ) {
            return $value->format( self::DEFAULT_DATE_FORMAT );
        }

        if ( is_string( $value ) ) {
            return preg_replace( '/[\r\n]+/', ' ', $value );
        }

        if ( is_array( $value ) || is_object( $value ) ) {
            return wp_json_encode( $value );
        }

        return (string) $value;
    }

    /**
     * Write the log message to the log file
     *
     * @since WPUF_SINCE
     *
     * @param string $message The formatted log message.
     * @param string $level   The log level.
     *
     * @return bool
     */
    protected function write( $message, $level = self::INFO ) {
        // file is not yet ready to write, cached for later
        if ( ! is_resource( $this->file_handle ) ) {
            $this->cache_log( $message, $level );

            return true;
        }

        fwrite( $this->file_handle, (string) $message ); // @codingStandardsIgnoreLine.

        return true;
    }

    /**
     * Opens the log for writing.
     *
     * @since WPUF_SINCE
     *
     * @return void
     */
    private function open() {
        $file_path   = $this->log_directory . '/' . $this->file_name;
        $needs_chmod = ! file_exists( $file_path );

        // create the log directory if not exists
        if ( ! is_dir( $this->log_directory ) ) {
            wp_mkdir_p( $this->log_directory );

            // prevent direct web access to log files
            @file_put_contents( $this->log_directory . '/.htaccess', "deny from all\n" ); // @codingStandardsIgnoreLine.

            // create a blank index file to prevent directory listing
            @file_put_contents( $this->log_directory . '/index.php', "<?php\n// Silence is golden.\n" ); // @codingStandardsIgnoreLine.
        }

        // open or create the logfile
        $fh = @fopen( $file_path, 'a' ); // @codingStandardsIgnoreLine.

        if ( $needs_chmod && $fh ) {
            @chmod( $file_path, $this->mode & ~umask() ); // @codingStandardsIgnoreLine.
        }

        $this->file_handle = $fh;
    }

    /**
     * Closes the log stream resource.
     *
     * @since WPUF_SINCE
     *
     * @return void
     */
    private function close() {
        if ( is_resource( $this->file_handle ) ) {
            fclose( $this->file_handle ); // @codingStandardsIgnoreLine.
            $this->file_handle = null;
        }
    }

    /**
     * Cache log to write later.
     *
     * @since WPUF_SINCE
     *
     * @param string $message Log entry text.
     * @param string $level   Log entry level.
     *
     * @return void
     */
    protected function cache_log( $message, $level ) {
        $this->cached_logs[] = [
            'message' => $message,
            'level'   => $level,
        ];
    }

    /**
     * Write cached logs. If a log is written too early in the request, pluggable functions may be unavailable.
     * These logs will be cached and written on 'init' action.
     *
     * @since WPUF_SINCE
     *
     * @return void
     */
    public function write_cached_logs() {
        $logs              = $this->cached_logs;
        $this->cached_logs = [];

        foreach ( $logs as $log ) {
            $this->log( $log['message'], $log['level'] );
        }
    }

    /**
     * Delete all logs older than a defined timestamp.
     *
     * @since WPUF_SINCE
     *
     * @param integer $timestamp Timestamp to delete logs before.
     *
     * @return void
     */
    public function delete_logs_before_timestamp( $timestamp = 0 ) {
        if ( ! $timestamp ) {
            return;
        }

        $log_files = $this->get_log_files();

        foreach ( $log_files as $log_file ) {
            $last_modified = filemtime( trailingslashit( $this->log_directory ) . $log_file );

            if ( $last_modified < $timestamp ) {
                @unlink( trailingslashit( $this->log_directory ) . $log_file ); // @codingStandardsIgnoreLine.
            }
        }
    }

    /**
     * Clear all logs older than a defined number of days. Defaults to 60 days.
     *
     * @since WPUF_SINCE
     *
     * @return void
     */
    public function clear_expired_logs() {
        $days      = absint( apply_filters( 'wpuf_logger_expiration_days', 60 ) );
        $timestamp = strtotime( "-{$days} days" );

        $this->delete_logs_before_timestamp( $timestamp );
    }

    /**
     * Get all log files in the log directory.
     *
     * @since WPUF_SINCE
     *
     * @return array
     */
    public function get_log_files() {
        $files  = @scandir( $this->log_directory ); // @codingStandardsIgnoreLine.
        $result = array();

        if ( ! empty( $files ) ) {
            foreach ( $files as $key => $value ) {
                if ( ! in_array( $value, array( '.', '..' ), true ) ) {
                    if ( ! is_dir( trailingslashit( $this->log_directory ) . $value ) && strstr( $value, '.log' ) ) {
                        $result[ sanitize_title( $value ) ] = $value;
                    }
                }
            }
        }

        return $result;
    }

    /**
     * Close the file when the object is destructed or the script is stopped or exited.
     *
     * @since WPUF_SINCE
     *
     * @return void
     */
    public function __destruct() {
        $this->close();
    }
}
