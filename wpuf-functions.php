...
function wpuf_user_can_edit_post( $post_id, $check_settings = true ) {
    ...
    if ( $current_user_id === $post_author_id ) {
        // User owns the post, check if they have edit_post capability
        ...
        // User doesn't own the post, check edit_others_posts capability
        $post_type_object = get_post_type_object( $post->post_type );

        if ( ! $post_type_object || ! current_user_can( $post_type_object->cap->edit_others_posts ) ) {
            return new WP_Error(
                'wpuf_unauthorized_edit_others',
                __( 'You are not authorized to edit this post.', 'wp-user-frontend' )
            );
        }

        // Double check with edit_post capability
        if ( ! current_user_can( 'edit_post', $post_id ) ) {
            return new WP_Error(
                'wpuf_unauthorized_edit',
                __( 'You are not authorized to edit this post.', 'wp-user-frontend' )
