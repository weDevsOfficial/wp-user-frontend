...
class Field_Manager {
    ...
    /**
     * Render multiple instances of form fields with values
     *
     * @param array  $fields
     * @param array  $values
     * @param int    $form_id
     ...
     * @return void
     */
    public function render_fields_with_values( $fields, $values, $form_id, $atts = [], $type = 'post', $post_id = NULL ) {
        if ( ! $fields || ! $values ) {
            return;
        }
        ...
        foreach ( $normalized_values as $instance_index => $instance_values ) {
            echo '<div class="wpuf-instance" data-instance="' . esc_attr( $instance_index ) . '">';
    ...
    /**
     * Render a single instance of form fields with values
     *
     * @param array  $fields
     * @param array  $values
     * @param int    $form_id
     ...
     * @return void
     */
    public function render_single_instance_with_values( $fields, $values, $form_id, $atts = [], $type = 'post', $post_id = NULL ) {
        if ( ! $fields || ! $values ) {
            return;
        }
        ...
        foreach ( $fields as $field ) {
            ...
            if ( $this->check_field_visibility( $field ) ) {
                if ( is_object( $field_object ) ) {
                    ...
                    // Render the field with the current value
                    $field_object->render( $field_with_value, $form_id, $type, $post_id );
                    $field_object->conditional_logic( $field_with_value, $form_id );
                }
            }
        }
     ...
     * @return array
     */
    public function normalize_values_for_fields( $fields, $values ) {
        $normalized_values = [];
