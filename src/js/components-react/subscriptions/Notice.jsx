...
export default function Notice({ message, type, index, onRemove }) {
    const id = `toast-${type}-${index}`;
    ...
    const iconClass = classes[type] || classes.danger;
    ...
    return (
        ...
        >
            <div className="wpuf-flex wpuf-items-center wpuf-justify-between">
                <div className={`${iconClass} wpuf-mr-2 wpuf-rounded-lg wpuf-flex wpuf-items-center wpuf-justify-center wpuf-w-8 wpuf-h-8`}>
                    {type === 'success' && (
                        <svg
                            className="wpuf-w-5 wpuf-h-5"
                            aria-hidden="true"
                ...
                <div className="ms-3 wpuf-text-sm wpuf-font-normal">{message}</div>
            </div>
            <button
                onClick={() => onRemove()}
                type="button"
