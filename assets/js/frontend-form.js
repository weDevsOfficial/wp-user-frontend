(function ($, window) {
    ...
    window.WP_User_Frontend = {
        init: function () {
            ...
            if (window.matchMedia("(max-width: 600px)").matches) {
                // Post toggle
                $("table.items-table tr td .post-edit-icon").click(
                    function (e) {
                        e.preventDefault();
                        $(this)
                            .parents("tr")
                            .find(".data-column")
                            .toggleClass("flex-column");
            ...
            $(".wpuf-form").on(
                "step-change-fieldset",
                function (event, number, step) {
                    if (wpuf_plupload_items.length) {
                        for (
                            var i = wpuf_plupload_items.length - 1;
                            i >= 0;
                            i--
                        ) {
                    ...
                    if (wpuf_map_items.length) {
                        for (var i = wpuf_map_items.length - 1; i >= 0; i--) {
                            google.maps.event.trigger(
                                wpuf_map_items[i].map,
                                "resize",
                            );
                            wpuf_map_items[i].map.setCenter(
                                wpuf_map_items[i].center,
                            );
                        }
                    }
                },
            ...
            // Twitter URL real-time validation
            $(".wpuf-form").on(
                "input blur",
                'input[data-type="twitter_url"]',
                this.validateTwitterField,
            );

            //comfirmation alert for canceling subscription
            $(':submit[name="wpuf_user_subscription_cancel"]').click(
                function (e) {
                    e.preventDefault();
        ...
        warningOnExit: function () {
            $('input[name="submit"], #wpuf-post-draft').on(
                "click",
                function () {
            ...
            $(".wpuf-form-add").on("change", function () {
                $(
                    ".wpuf-form-add input, .wpuf-form-add select, .wpuf-form-add textarea",
                ).each(function (index) {
                    if (
        ...
        enableMultistep: function (o) {
            var js_obj = this;
            var step_number = 0;
            var progressbar_type = $(
                ':hidden[name="wpuf_multistep_type"]',
            ).val();
            ...
            $("fieldset.wpuf-multistep-fieldset")
                .find(".wpuf-multistep-prev-btn")
                .first()
                .remove();
            $("fieldset.wpuf-multistep-fieldset")
                .find(".wpuf-multistep-next-btn")
                .last()
                .remove();
            ...
            if (
                ...
                var firstLegend = $(
                    "fieldset.wpuf-multistep-fieldset legend",
                ).first();
                ...
                $(".wpuf-form").each(function () {
                    ...
                    $(".wpuf-step-wizard li", progressbar)
                        .first()
            ...
            $(
                "fieldset .wpuf-multistep-prev-btn, fieldset .wpuf-multistep-next-btn",
            ).click(function (e) {
                // js_obj.formSubmit();
                if ($(this).hasClass("wpuf-multistep-next-btn")) {
                    var result = js_obj.formStepCheck(
                        "",
        ...
        change_fieldset: function (step_number, progressbar_type) {
            var current_step = $("fieldset.wpuf-multistep-fieldset").eq(
                step_number,
            );
            ...
            $(".wpuf-step-wizard li").each(function () {
                if ($(this).index() <= step_number) {
                    progressbar_type == "step_by_step"
                        ...
                        : $(".wpuf-ps-bar", this).addClass(
                              "passed-wpuf-ms-bar",
                          );
                } else {
                    progressbar_type == "step_by_step"
                        ? $(this).removeClass("passed-wpuf-ms-bar")
                        : $(".wpuf-ps-bar", this).removeClass(
                              "passed-wpuf-ms-bar",
                          );
                }
            });

            $(".wpuf-step-wizard li").removeClass(
                "wpuf-ms-bar-active active-step completed-step",
            );
            $(".passed-wpuf-ms-bar")
            ...
            if (
                ...
                var progress_percent =
                    ((step_number + 1) * 100) /
                    $("fieldset.wpuf-multistep-fieldset").length;
                var progress_percent = Number(progress_percent.toFixed(2));
                $(".wpuf-multistep-progressbar").progressbar({
                    value: progress_percent,
                });
                $(".wpuf-progress-percentage").text(
                    legend + " (" + progress_percent + "%)",
                );
            }
        ...
        getChildCats: function (dropdown, level, taxonomy, form_id) {
            ...
            var taxonomy =
                typeof taxonomy !== "undefined" ? taxonomy : "category";
        ...
        formSubmit: function (e) {
            ...
            if (form_data) {
                // send the request
                form.find("li.wpuf-submit").append(
                    '<span class="wpuf-loading"></span>',
                );
                submitButton
                ...
                $.post(wpuf_frontend.ajaxurl, form_data, function (res) {
                    ...
                    if (res.success) {
                        ...
                        if (
                            typeof res.data.type !== "undefined" &&
                            res.data.type === "login"
                        ) {
                            Swal.fire({
                                text: res.error,
        ...
        validateForm: function (self) {
            ...
            var visibleFields = self.find(
                "input:visible, textarea:visible, select:visible, div.br-widget:visible, .wpuf-rich-validation:visible, div.wpuf-attachment-upload-filelist:visible",
            );
            ...
            visibleFields.each(function (i, item) {
                ...
                switch (dataType) {
                    case "rich":
                        ...
                        var isRestrictionFailed =
                            WP_User_Frontend.editorLimit.isRestrictionFailed(
                                item,
                            );
                        ...
                        var containingShortcode =
                            WP_User_Frontend.editorContainingShortcode(item);
                        ...
                        if (containingShortcode.shortcodeFound) {
                            errors.push({
                                ...
                                container: item,
                                message:
                                    wpuf_frontend.protected_shortcodes_message.replace(
                                        "%shortcode%",
                                        "[" +
                                            containingShortcode.shortcode +
                                            "]",
                                    ),
                            });
                        ...
                        richTexts.push(
                            item_name + "=" + encodeURIComponent(val),
                        );
                    ...
                    case "text":
                        ...
                        if (WP_User_Frontend.isTelephoneField(item)) {
                            ...
                            var iti =
                                window.intlTelInputGlobals.getInstance(item);
                            if (
                                "" !== iti.getNumber() &&
                                !iti.isValidNumber()
                            ) {
                    ...
                    case "twitter_url":
                        ...
                        if (
                            val !== "" &&
                            !WP_User_Frontend.isValidTwitterURL(val)
                        ) {
                            errors.push({
                                type: "validation",
                                container: item,
                    ...
                    case "confirm_password":
                        ...
                        if (hasRepeat === "yes") {
                            var repeatItem = $(item)
                                .closest(".wpuf-form")
                                .find('[data-type="confirm_password"]')
                                .eq(0);
                        ...
                        if (strength) {
                            var strengthMeter = wp.passwordStrength.meter(
                                val,
                                wp.passwordStrength.userInputDisallowedList(),
                            );
                            ...
                            if (strength === "weak" && strengthMeter < 2) {
                                errors.push({
                                    ...
                                    container: item,
                                    message:
                                        wpuf_frontend.password_warning_weak,
                                });
                            } else if (
                                strength === "medium" &&
                                strengthMeter < 3
                                ...
                                errors.push({
                                    ...
                                    container: item,
                                    message:
                                        wpuf_frontend.password_warning_medium,
                                });
                            } else if (
                                strength === "strong" &&
                                strengthMeter < 4
                                ...
                                errors.push({
                                    ...
                                    container: item,
                                    message:
                                        wpuf_frontend.password_warning_strong,
                                });
                            }
                        }
                    ...
                    default: // Google map etc.
                        ...
                        if (
                            $(item).attr("id") &&
                            $(item).attr("id").startsWith("wpuf-map")
                        ) {
                            var mapContainer = $(item).parents(
                                ".wpuf-form-google-map-container",
                            );
                            mapField = $(mapContainer).find(
                                'input[id^="wpuf-map-lat"]',
                            );
            ...
            var conditionallyHiddenFields = self.find(
                'li.wpuf-el[style*="display: none"] input',
            );
        ...
        markError: function (item, error_type, error_message) {
            ...
            switch (error_type) {
                case "required":
                    error_string =
                        error_string + " " + error_str_obj[error_type];
                    break;
                case "mismatch":
                    error_string =
                        error_string + " " + error_str_obj[error_type];
            ...
            if ($(item).closest(".wpuf-fields.wpuf-fields-address").length) {
                $(item)
                    .closest("div.wpuf-sub-fields")
                    .find("div.wpuf-error-msg")
                    .remove();
                $(item)
                    .closest("div.wpuf-sub-fields")
                    .append(
                        '<div class="wpuf-error-msg">' +
                            error_string +
                            "</div>",
                    );
            } else {
                $(item)
                    .closest("div.wpuf-fields")
                    .find("div.wpuf-error-msg")
                    .remove();
                $(item)
                    .closest("div.wpuf-fields")
        ...
        editorLimit: {
                ...
                if (type === "no") {
                    ...
                    setTimeout(function () {
                        tinyMCE.get(field).onKeyUp.add(function (ed, event) {
                            WP_User_Frontend.editorLimit.tinymce.onKeyUp(
                                ed,
                                event,
                                limit,
                                limit_type,
            ...
            checkTinyMCERestriction: function (field, data, ed) {
                ...
                var limit_type =
                    typeof data[3] !== "undefined" && data[3] !== null
                        ...
                        : "";
                var limit_to =
                    typeof data[4] !== "undefined" && data[4] !== null
                        ? data[4].trim()
                        : "";
                var limit_label = "";
                ...
                if ("word" === limit_type) {
                    numWords =
                        WP_User_Frontend.editorLimit.tinymce.getStats(ed).words;
                    limit_label = "word_" + limit_to;
                } else {
                    numWords =
                        WP_User_Frontend.editorLimit.tinymce.getStats(ed).chars;
                    limit_label = "char_" + limit_to;
                }

                var errorMessage =
                    wpuf_frontend["word_" + limit_to] + " " + limit;
            ...
            isRestrictionFailed: function (field) {
                ...
                if (isTinymce) {
                    return WP_User_Frontend.editorLimit.checkTinyMCERestriction(
                        field,
                        data,
                        ed,
                    );
            ...
            tinymce: {
                ...
                onKeyUp: function (ed, event, limit, limit_type, limit_to) {
                    ...
                    if (
                        $(field.closest(".wpuf-fields")).hasClass("has-error")
                    ) {
                    ...
                    if (!numChars) {
                        field
                            .closest(".wpuf-fields")
                            .find("span.wpuf-wordlimit-message")
                            .html("");
                    ...
                    if ("word" === limit_type) {
                        numChars =
                            WP_User_Frontend.editorLimit.tinymce.getStats(
                                ed,
                            ).words;
                    }
                ...
                onPaste: function (ed, event, limit, limit_type, limit_to) {
                    ...
                    WP_User_Frontend.editorLimit.make_media_embed_code(
                        editorContent,
                        ed,
                    );
                },
            ...
            make_media_embed_code: function (content, editor) {
                $.post(
                    ...
                    function (data) {
                        editor.setContent(
                            editor.getContent() + editor.setContent(data),
                        );
                    },
                );
            },
        ...
        contentLimitMessage: function (
            ...
            if ("word" === content_limit_type) {
                if (limit_to === "min") {
                    ...
                    limitMessage += wpuf_frontend.word_min_details.replace(
                        "%number%",
                        limit,
                    );
                } else {
                    limitMessage = wpuf_frontend.word_max_title + "<br>";
                    limitMessage += wpuf_frontend.word_max_details.replace(
                        "%number%",
                        limit,
                    );
                }
                ...
                if (limit_to === "min") {
                    ...
                    limitMessage += wpuf_frontend.char_min_details.replace(
                        "%number%",
                        limit,
                    );
                } else {
                    limitMessage = wpuf_frontend.char_max_title + "<br>";
                    limitMessage += wpuf_frontend.char_max_details.replace(
                        "%number%",
                        limit,
                    );
                }
    ...
    $(function () {
        ...
        if (
            ...
            var el = $("ul.wpuf-payment-gateways li").find(
    ...
    $(function () {
        $('input[name="first_name"], input[name="last_name"]').on(
            "change keyup",
            function () {
                var myVal,
                    newVal = $.makeArray(
    ...
    $(function ($) {
        $('.wpuf-form-add input[name="dokan_store_name"]').on(
            "focusout",
            function () {
                var value = $(this)
                    .val()
        ...
        $('.wpuf-form-add input[name="shopurl"]').keydown(function (e) {
            ...
            if (
                $.inArray(
                    e.keyCode,
                    [46, 8, 9, 27, 13, 91, 109, 110, 173, 189, 190],
                ) !== -1 ||
                // Allow: Ctrl+A
    ...
    $(function ($) {
        ...
        $(document).on("click", ".wpuf-eye", function () {
            ...
            if (input.attr("type") === "password") {
                ...
                $(this).attr(
                    "src",
                    wpuf_frontend.asset_url + "/images/eye-close.svg",
                );
            } else {
                input.attr("type", "password");
                $(this).attr(
                    "src",
                    wpuf_frontend.asset_url + "/images/eye.svg",
                );
            }
        });
    });
