...
export default function loginAndSetupTests() {
     ...
     *  
     */
        test('LS0001 : Admin is logging into Admin-Dashboard', { tag: ['@Both'] }, async () => {
            const BasicLogin = new BasicLoginPage(page);
            await BasicLogin.basicLogin(Users.adminUsername, Users.adminPassword);
        });

        test('LS0002 : Admin is checking Dashboard page reached', { tag: ['@Both'] }, async () => {
            const BasicLogin = new BasicLoginPage(page);
            await BasicLogin.validateBasicLogin();
        });

        test('LS0003 : Admin is checking Plugin Status - Lite Activation', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.pluginStatusCheckLite();
        });

        test('LS0004 : Admin is checking Plugin Status - Pro Activation', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.pluginStatusCheckPro();
        });

        test('LS0005 : Admin is activating license - Pro', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.licenseActivateWPUFPro();
        });

        test('LS0006 : Admin is Completing WPUF setup', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.wpufSetup();
        });

        test('LS0007 : Admin is visiting WPUF Page', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.pluginVisitWPUF();
        });

        test('LS0008 : Admin is validating WPUF Pages', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.validateWPUFpages();
        });

        test('LS0009 : Admin is validating WPUF Pages from frontend', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.validateWPUFpagesFE();
        });

        test('LS0010 : Admin is validating account page tabs from frontend', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.validateAccountPageTabs();
        });

        test('LS0011 : Admin is changing WPUF Settings', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.changeSettingsSetLoginPageDefault();
        });

        test('LS0012 : Admin is setting Permalink', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.setPermalink();
        });

        test('LS0013 : Admin is allowing anyone to register', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.allowRegistration();
        });

        test('LS0014 : Admin is creating a New User', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
        ...
        test('LS0015 : Admin is adding post categories', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.createPostCategories();
        });

        test('LS0016 : Admin is adding post tags', { tag: ['@Both'] }, async () => {
            const SettingsSetup = new SettingsSetupPage(page);
            await SettingsSetup.createPostTags();
        });

    test('LS0017 : Admin is adding credentils for Google Map', { tag: ['@Both'] }, async () => {
        const SettingsSetup = new SettingsSetupPage(page);
        const googleMapAPIKey = process.env.GOOGLE_MAP_API_KEY;
        await SettingsSetup.addGoogleMapAPIKey(googleMapAPIKey?.toString() || '');
    });

    test('LS0018 : Admin is adding credentils for ReCaptcha', { tag: ['@Both'] }, async () => {
        const SettingsSetup = new SettingsSetupPage(page);
    ...
    test('LS0019 : Admin is adding credentils for Cloudflare Turnstile', { tag: ['@Both'] }, async () => {
        const SettingsSetup = new SettingsSetupPage(page);
        ...
        test('LS0020 : Admin is logging out successfully', { tag: ['@Both'] }, async () => {
