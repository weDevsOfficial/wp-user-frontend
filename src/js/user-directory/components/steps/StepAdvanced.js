...
const StepAdvanced = ({ formData, setFormData, config }) => {
    ...
    const freeAvatarSizes = ['128']; // Only 128 is free
            ...
            <div className="wpuf-mt-8">
                ...
                <div className="wpuf-mt-[25px]">
                    ...
                    <input
                        ...
                        value={formData.users_per_page || 10}
                ...
                {/* Default Sort - Side by Side Layout (matching Pro) */}
                <div className="wpuf-mt-[25px]">
                    <label className="wpuf-flex wpuf-text-left wpuf-text-sm wpuf-font-medium wpuf-text-gray-700 wpuf-mb-3">
                        {__('Default Sort', 'wp-user-frontend')}
                        <Tooltip className="wpuf-ml-2" content={__('Select the field by which users will be sorted by default and the sort order', 'wp-user-frontend')}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.375 9.375L9.40957 9.35771C9.88717 9.11891 10.4249 9.55029 10.2954 10.0683L9.70458 12.4317C9.57507 12.9497 10.1128 13.3811 10.5904 13.1423L10.625 13.125M17.5 10C17.5 14.1421 14.1421 17.5 10 17.5C5.85786 17.5 2.5 14.1421 2.5 10C2.5 5.85786 5.85786 2.5 10 2.5C14.1421 2.5 17.5 5.85786 17.5 10ZM10 6.875H10.0063V6.88125H10V6.875Z" stroke="#9CA3AF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path></svg>
                        </Tooltip>
                    </label>
                    <div className="wpuf-grid wpuf-grid-cols-2 wpuf-gap-3">
                        ...
                        <SingleSelect
                            options={[
                                { value: 'asc', label: __('ASC', 'wp-user-frontend') },
                                { value: 'desc', label: __('DESC', 'wp-user-frontend') }
                            ]}
                            value={formData.default_sort_order || formData.order || 'desc'}
                            onChange={(value) => handleChange({ target: { name: 'default_sort_order', value } })}
                        />
                    </div>
                </div>
                ...
                <div className="wpuf-mt-[25px]">
                    <div className="wpuf-flex wpuf-items-center wpuf-gap-2 wpuf-mb-3">
                        <label className="wpuf-flex wpuf-text-left wpuf-text-sm wpuf-font-medium wpuf-text-gray-700 wpuf-m-0">
                ...
                <div className="wpuf-mt-[25px]">
                    ...
                    {formData.directory_layout && (
                        <div className="wpuf-mb-3 wpuf-flex wpuf-items-center wpuf-gap-2">
                            <div
                                className="wpuf-flex wpuf-items-center wpuf-flex-1"
                                style={{
                                    maxWidth: '793px',
                                    height: '52px',
                                    borderRadius: '6px',
                    ...
                    <div className="wpuf-flex wpuf-flex-wrap wpuf-gap-2">
                        {avatarSizes.map(size => {
                            const isSelected = (formData.avatar_size || '128') === size.value;
                            const isHovered = hoveredOption === `avatar-${size.value}`;

                            return (
                                <label
                                    key={size.value}
                                    className={`wpuf-relative wpuf-flex wpuf-flex-col wpuf-items-center wpuf-justify-center wpuf-w-[103px] wpuf-rounded-lg wpuf-transition-all wpuf-p-3 wpuf-pb-1 wpuf-gap-1 ${
                                        !size.isFree
                                            ? `wpuf-cursor-not-allowed wpuf-opacity-60 hover:wpuf-opacity-80 wpuf-bg-gray-50`
                                            : isSelected
                                    ...
                                    onMouseLeave={() => !size.isFree && setHoveredOption(null)}
                                >
                                    <input
                                        type="radio"
                                        name="avatar_size"
